...
namespace {
    ...
    // V25 regression: the production screenshots showed equivalent English and
    // Taglish SAR evidence diverging because one paraphrase had an extra outcome
    // qualifier and more raw action verbs. Those are presentation differences, not
    // different evidence strength.
    $screenEnglish = $problem->invoke($service, 12, 22, 18, 1, 1, 2, 1, false, false, 3);
    $screenTaglish = $problem->invoke($service, 12, 25, 16, 1, 4, 1, 0, false, false, 3);
    $check($screenEnglish['score'] === $screenTaglish['score'],
        'V25 screenshot regression: equivalent English/Taglish SAR evidence has identical score',
        ['english'=>$screenEnglish['score'],'taglish'=>$screenTaglish['score']]);
    $check($screenEnglish['specificity'] === $screenTaglish['specificity'],
        'V25 screenshot regression: raw verb/qualifier count does not change specificity',
        ['english'=>$screenEnglish['specificity'],'taglish'=>$screenTaglish['specificity']]);

    $serviceSource = file_get_contents(__DIR__ . '/../app/Services/AssessmentInsightService.php');
    $check(str_contains($serviceSource, '$appliedEvidenceUnits = 0;'),
        'V25 skills regression: applied semantic evidence cannot double-count an already matched competency');

    echo "\nAll V25 language-neutral scoring helper checks passed.\n";
}
